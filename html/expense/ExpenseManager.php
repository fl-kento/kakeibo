...
class ExpenseManager {
  ...
  public function getType() {
    ...
    $expense_type = $this->db->query($sql, $params);
    return $expense_type;
  }
  public function getExpense($month, $year) {
    $sql = 'SELECT SUM(amount) AS 集計金額, name, type_no FROM expense INNER JOIN type ON expense.type_no = type.id WHERE expense.user_id = :id AND month(date) = :month AND year(date) = :year GROUP BY type_no';    
    ...
    $sql = 'SELECT expense.user_id, expense_no, name, amount FROM expense INNER JOIN type ON expense.type_no = type.id WHERE expense.user_id = :id 
  ...
  public function getExpenseDetail($month, $year) {   
    $sql = 'SELECT expense_no, amount, date, content FROM expense INNER JOIN type ON expense.type_no = type.id WHERE expense.user_id = :id AND type_no = :no AND month(date) = :month AND year(date) = :year ORDER BY date DESC';
  ...
  public function getNowContent() {
    $sql = 'SELECT amount, type_no, date, content FROM expense INNER JOIN type ON expense.type_no = type.id WHERE expense.user_id = :id AND expense_no = :no';
